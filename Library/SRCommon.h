...
*/
FOUNDATION_STATIC_INLINE NSBundle *SRBundle(void)
{
...
{
    // JKG: Modified to grab strings from the main bundle instead of the framework
    NSString *str = NSLocalizedStringFromTableInBundle(aKey, @"ShortcutRecorder", [NSBundle mainBundle], nil);
    if(str.length == 0) str = NSLocalizedStringFromTableInBundle(aKey, @"ShortcutRecorder", SRBundle(), nil);
    return str;
}
